/// Modified by Bridger Holt for Fall 2018 CS 3500.
...
namespace SS
{
    ...
    /// <summary>
    /// The type of delegate used for CellDoubleClick events.
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="col"></param>
    /// <param name="row"></param>
    public delegate void DoubleClickHandler(SpreadsheetPanel sender, int col, int row);


    /// <summary>
    /// The type of delegate used for Scrolled events.
    /// </summary>
    /// <param name="sender"></param>
    public delegate void ScrollHandler(SpreadsheetPanel sender);
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// Calculates the absolute position of the top-left point of a cell.
        /// </summary>
        /// <param name="col"></param>
        /// <param name="row"></param>
        /// <returns></returns>
        public bool GetCellTopLeft(int col, int row, out Point point)
        {
            var inBounds = drawingPanel.GetCellPosition(col, row, out point);
        ...
        /// <summary>
        /// Calculates the absolute position of the center of a cell.
        /// </summary>
        /// <param name="col"></param>
        /// <param name="row"></param>
        /// <returns></returns>
        public bool GetCellCenter(int col, int row, out Point point)
        {
            var inBounds = GetCellTopLeft(col, row, out point);
            point.Offset(DATA_COL_WIDTH / 2, DATA_ROW_HEIGHT / 2);
            return inBounds;
        }
        ...
        /// <summary>
        /// Sends notifications when a cell is double clicked.
        /// </summary>
        public event DoubleClickHandler CellDoubleClicked;

        /// <summary>
        /// Sends notifications when the field is scrolled.
        /// </summary>
        ...
        private class DrawingPanel : Panel
        {
            ...
            public bool GetCellPosition(int col, int row, out Point point)
            {
                var x = LABEL_COL_WIDTH + (col - _firstColumn) * DATA_COL_WIDTH;
                var y = LABEL_ROW_HEIGHT + (row - _firstRow) * DATA_ROW_HEIGHT;

                point = new Point(x, y);

                if (x >= LABEL_COL_WIDTH && y >= LABEL_ROW_HEIGHT)
                {
                    return true;
                }
            ...
            protected override void OnMouseClick(MouseEventArgs e)
            {
                ...
                int col, row;
                if (GetColRowOfMouse(e.X, e.Y, out col, out row))
                {
                  _selectedCol = col;
                  _selectedRow = row;
            ...
            private void ClickHandler(MouseEventArgs e)
            {
                ...
                if (GetColRowOfMouse(e.X, e.Y, out col, out row))
                {
                    _selectedCol = col;
                    _selectedRow = row;
                    _ssp.CellDoubleClicked?.Invoke(_ssp, col, row);
                }
            ...
            /// <summary>
            /// Returns true if the mouse is on a cell,
            /// sets the cell's col and row as output parameters.
            /// </summary>
            ...
            /// <param name="row"></param>
            /// <returns></returns>
            private bool GetColRowOfMouse(int mouseX, int mouseY, out int col, out int row)
            {
                if (mouseX > LABEL_COL_WIDTH && mouseY > LABEL_ROW_HEIGHT)
                {
                    int x = (mouseX - LABEL_COL_WIDTH) / DATA_COL_WIDTH;
                    int y = (mouseY - LABEL_ROW_HEIGHT) / DATA_ROW_HEIGHT;
